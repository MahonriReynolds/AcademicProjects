...
function addItem(products) {
    ...
    const newItemHTML = `
        ...
        <label for="order-date-${itemCount}">Order Date</label>
        <input type="date" id="order-date-${itemCount}" name="order-date-${itemCount}" required>

        <label for="expiration-date-${itemCount}">Expiration Date</label>
        <input type="date" id="expiration-date-${itemCount}" name="expiration-date-${itemCount}">
    ...
    // Get the product select element for this item
    const productSelect = newItem.querySelector(`#product-${itemCount}`);

    // Add event listener to update the unit when a product is selected
...
function collectFormData() {
    const items = itemsList.map(item => {
        ...
        const orderDateInput = item.querySelector('input[name^="order-date"]');
        const expirationDateInput = item.querySelector('input[name^="expiration-date"]');
